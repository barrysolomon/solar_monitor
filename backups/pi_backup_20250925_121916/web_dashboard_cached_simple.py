...
def get_page_content(page):
    if page == 'overview':
        ...
        <div class="page-header">
            ...
            <p>Database tools, SQL queries, and data export</p>
        </div>
        ...
        <div class="info-card">
            ...
            <div id="db-stats">
                ...
                <p>Latest Record: <span id="latest-record">Loading...</span></p>
                <p>Date Range: <span id="date-range">Loading...</span></p>
            </div>
            <button class="btn" onclick="refreshDbStats()">🔄 Refresh Stats</button>
        </div>
        
        <div class="info-card">
            <h3>🔍 SQL Query Interface</h3>
            <div class="sql-interface">
                <textarea id="sql-query" class="sql-textarea" placeholder="Enter your SQL query here...">SELECT * FROM solar_data ORDER BY timestamp DESC LIMIT 10;</textarea>
                <div class="query-controls">
                    ...
                    <button class="btn" onclick="loadSampleQueries()">📋 Sample Queries</button>
                </div>
            </div>
...
def get_page_script(page):
    if page == 'overview':
        ...
        async function refreshDbStats() {
            try {
                const response = await fetch('/api/db/detailed-status');
                const data = await response.json();
                ...
                if (data.success) {
                    ...
                    document.getElementById('latest-record').textContent = 
                        data.latest_timestamp || 'Unknown';
        ...
        function loadSampleQueries() {
            const samples = [
                ...
                'SELECT DATE(timestamp) as date, AVG(production_kw) as avg_production FROM solar_data GROUP BY DATE(timestamp) ORDER BY date DESC LIMIT 7;',
                'SELECT COUNT(*) as total_records, MIN(timestamp) as first_record, MAX(timestamp) as last_record FROM solar_data;',
                'SELECT * FROM solar_data WHERE production_kw > 3.0 ORDER BY timestamp DESC LIMIT 20;'
            ];
        ...
        async function exportData(format) {
            ...
            try {
                const response = await fetch(`/api/export-data?format=${format}&limit=${limit}`);
                ...
                if (response.ok) {
                    ...
                    a.href = url;
                    a.download = `solar_data_${new Date().toISOString().split('T')[0]}.${format}`;
                    document.body.appendChild(a);
                    a.click();
                    window.URL.revokeObjectURL(url);
        ...
        if (document.getElementById('db-stats')) {
            refreshDbStats();
        }
        '''
    elif page == 'system':
